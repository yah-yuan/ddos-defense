...
FromHost(fakeDev, 192.168.3.134/24, ETHER 00-01-02-03-04-05)
  -> lhcl :: Classifier(12/0800, 12/0806) //[0]IP, [1]ARP querier
  -> Strip(14)
  -> CheckIPHeader()
  -> DropBroadcasts
  -> lhipc :: IPClassifier(port 33333)
...
dt[1] -> ICMPError(192.168.3.134, timeexceeded) 
  -> arpq

fr[1] -> ICMPError(192.168.3.134, unreachable, needfrag) 
  -> arpq

gio[1] -> ICMPError(192.168.3.1 36, parameterproblem) -> arpq;

toh :: IPPrint(ToHost) -> ToHost(fakeDev)

lhcl[1]
  -> lharpr ::ARPResponder(192.168.3.0/24 00-01-02-03-04-05)
